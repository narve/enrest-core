...
import no.dv8.rest3.EnrestConfigurator;
...
@javax.servlet.annotation.WebServlet(urlPatterns = {"/rest", "/rest/*"})
public class EnrestServlet extends HttpServlet {

    Enrest enrest = EnrestConfigurator.getEnrest();
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String _path = url.substring("/rest".length());
        if (_path.startsWith("/"))
...
//        if ("/rest".equals(url)) {
//            String html = html(getRestContainer().linksFrom(Void.class));
//            outputStream.print(html);
//        } else {
//            Optional<Transition> first = container.transitions.stream().filter(t -> t.getUrl().equals(url)).findFirst();
//            if (!first.isPresent()) {
//                throw new NullPointerException("Dont know how to handle " + url);
//            } else {
//                Object apply = first.get().getAction().apply(extractParams(req, first.get()));
//                String html = html(apply);
//                outputStream.print(html);
//            }
//        }

//        url = url.substring( "/rest".length() );

        if (path.isEmpty()) {
            for (Element e : enrest.index()) {
                outputStream.print(e.toHTML());
            }
        } else {
            Optional<EnrestResource> first = enrest.list().stream().filter(t -> matches(t, path) != null).findFirst();
            if (!first.isPresent()) {
                throw new NullPointerException("Dont know how to handle " + path);
            }
