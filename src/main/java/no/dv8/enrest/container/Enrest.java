...
public class Enrest {
    ...
    public Enrest add(Resource<?> resource) {
        resources.add( resource );
        return this;
    }

    public <T> Resource<T> locateResource(String resourceType) {
        return resources.stream().filter( r -> r.getName().equals( resourceType ) ).findFirst().get();
    }
...
//              .add(
//              new div().add(
//                new input().type("submit").stringValue("GO")
//              ))
//          );
////        return new a(r.getTo().getSimpleName()).href(r.getPath()).rel(relTypeProvider.reltype(r.getTo()));
//    }
